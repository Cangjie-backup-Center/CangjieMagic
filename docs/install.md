...
**⚠️注意**
目前代码仅在 Cangjie LTS 通用版（即 1.0.0）和 鸿蒙外发版（即 0.53.18）上能够正确编译执行。如果使用了其他版本的 Cangjie SDK，可能需要直接修改 Cangjie Magic 源代码进行适配。

**⚠️ Note**  
The code currently compiles and runs correctly only on **Cangjie LTS General Edition (v1.0.0)** and **HarmonyOS Release Edition (v0.53.18)**. If you use other versions of the Cangjie SDK, you may need to modify the Cangjie Magic source code directly to adapt.  
...
**步骤** 3️⃣: 在依赖 Cangjie Magic 的项目中配置 `cjpm.toml`

**Step** 3️⃣: Set the `cjpm.toml` of your project that uses Cangjie Magic
