...
@agent class Foo {
  @execution(
    plan |> repeat(think |> action, until: "Task is done") |> answer
  )
}
```
...
| `plan`      | Create a plan |
| `repeat`      | Loop internal operation sequences |
...
@agent class ResearchAssistant {
  @execution(
    orchestrate(action(tool: web_search)) |> answer
  )
  @tool
  func web_search(...) { ... }
}
...
| Operator      | Function             |
|-------------|----------------|
| `orchestrate` | Split the task into subproblems via LLM, with the number of subproblems determined automatically. |
...
@agent class Assistant {
  @execution(
    decide(
      when("Is the question about weather?") |> action(tool: weather_api),
      when("Is the question about order inquiry?") |> action(tool: db_query |> db_summary),
...
- `decide` accepts multiple `when` clauses.
- Each `when` clause consists of a condition (expressed in natural language) and an operation sequence.
    - When the condition in `when` is met (based on the current execution state), the corresponding operation sequence continues.
    - `when` clauses are executed top-down.
